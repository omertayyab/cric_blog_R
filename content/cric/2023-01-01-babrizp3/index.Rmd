...
However, when we multiply variables like this, we have to be wary of them having different scales. In our case, the PP scores are in tens while wickets are in ones, so one variable will have a larger impact on the product. To reduce this, we'll normalize the variables according to the following formula:

*Scaled Value = (Regular Value - sample mean)/std error*
...
cor.test(ds$scaled_PP_wkt, ds$scaled_PP_score)
```
...
tibble( errors = residuals(model_lm)) %>%
  ggplot(aes(x=errors)) + 
  geom_histogram(aes(y=..density..),stat="bin", bins=50, color="black", fill="gray") +
  geom_density(alpha=0.6, fill="gray") + 
  theme_economist()
shapiro.test(residuals(model_lm))
```
The null hypothesis of normality is accepted at 5% significance level, i.e. p-value > 5%, so our model is considered valid