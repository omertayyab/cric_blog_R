...
However, we have to limit ourselves to first innings only because during the 2nd innings, the final score is an upper bound and also a big factor in determining the chasing team's approach. We want to avoid that biasing our sample.
...
Now that we have dealt with the issue of correlated variables, we can turn our attention to the type of regression that'll be best for our purposes. Let's first take a look at the data visually to get an idea of what we're dealing with.
``` {r include=TRUE}
ds %>% ggplot() +
    geom_point(aes(x = PP_score, y = final)) 
```
