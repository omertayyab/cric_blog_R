...
model_knn <- caret::knnreg(formula=final~t,data=train)
model_svm <- e1071::svm(final~PP_score+PP_wkt,data=train, kernel="radial", epsilon=.1)
model_gam<- mgcv::gam(final~s(t, bs='ps', sp=0.6), data=train)
...
all_errors <- lapply(all_mods,pred_acc,test) %>% bind_cols

# get table
all_errors %>% mutate_all(abs) %>% summarize_all(median)
all_errors %>% mutate_all(abs) %>% summarize_all(mean)
